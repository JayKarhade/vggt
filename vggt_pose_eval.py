...
def vggt_inference(model, pairs, device, dtype, batch_size=1, verbose=True):
    """
    Perform inference using the VGGT model on a list of image pairs
    """

    if verbose:
        print(f'Running inference on {len(pairs)} pairs with batch size {batch_size}')
    ...
    for i in tqdm(range(0, len(pairs), batch_size)):
        ...
        for pair in batch_pairs:
            ...
            images_paths = [img1_path, img2_path]

            # Predict 3D points for this pair
            images = load_and_preprocess_images(images_paths).to(device)

        with torch.no_grad():
            with torch.cuda.amp.autocast(dtype=dtype):
                images = images[None]  # add batch dimension (N, 3, H, W) -> (1, N, 3, H, W)
            ...
            # Use identity extrinsic for ego-centric scene flow
            extrinsic = torch.eye(3, 4).unsqueeze(0).unsqueeze(0).expand(1, 2, 3, 4)
            ...
            depth_conf = depth_conf.to('cpu')

            # Construct 3D Points from Depth Maps and Cameras
            pointmaps = unproject_depth_map_to_point_map(
                depth_map.squeeze(0), 
                extrinsic.squeeze(0), 
                intrinsic.squeeze(0)
            ).astype(np.float32)

            # pointmaps = cv2.resize(pointmaps, (images.shape[-1], images.shape[-2]), interpolation=cv2.INTER_LINEAR)
            pointmaps = torch.tensor(pointmaps)

            # Invert extrinsics to get camera from world
            extrinsic = closed_form_inverse_se3(extrinsic.squeeze(0))

            H, W = img1['img'].shape[-2:]
            # Resize pointmaps to match the original image size
            pointmaps = pointmaps.permute(0, 3, 1, 2)  # [B, H, W, 3] -> [B, 3, H, W]
            pointmaps = torch.nn.functional.interpolate(pointmaps, size=(H, W), mode='bilinear', align_corners=False)
            pointmaps = pointmaps.permute(0, 2, 3, 1)  # [B, 3, H, W] -> [B, H, W, 3]

            # Resize depth_conf to match the original image size
            depth_conf = torch.nn.functional.interpolate(
                depth_conf, size=(H, W), mode='bilinear', align_corners=False
            )
            depth_conf = depth_conf.squeeze(0)

            pred1 = {
                'pts3d': pointmaps[0].unsqueeze(0),  # Add batch dimension
                'conf': depth_conf[0].unsqueeze(0),}
            pred2 = {
                'pts3d_in_other_view': pointmaps[1].unsqueeze(0),  # Add batch dimension
                'conf': depth_conf[1].unsqueeze(0),}
    ...
    result = collate_with_cat(results, lists=False)
    return result
...
def eval_pose_estimation_dist(args, model, device, img_path, save_dir=None, mask_path=None):
    ...
    for seq in tqdm(seq_list):
        try:
            ...
            # VGGT inference
            output = vggt_inference(
                vggt_model, pairs, device=device, dtype=vggt_dtype, batch_size=1, verbose=not silent
            )
